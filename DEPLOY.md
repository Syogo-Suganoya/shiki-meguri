...
printf '%s' "$GMI_API_KEY"     | gcloud secrets create gmi-api-key      --data-file=-
```
...
for s in gemini-api-key youcam-api-key youcam-secret ekispert-api-key gmi-api-key; do
...
```bash
  --set-env-vars="GEMINI_MODE=live,YOUCAM_MODE=live,EKISPERT_MODE=live,GMI_MODE=live,EKISPERT_MCP_URL=https://…" \
  --set-secrets="GEMINI_API_KEY=gemini-api-key:latest,YOUCAM_API_KEY=youcam-api-key:latest,YOUCAM_SECRET_KEY=youcam-secret:latest,EKISPERT_API_KEY=ekispert-api-key:latest,GMI_API_KEY=gmi-api-key:latest"
```
...
3. YouCam・駅すぱあと・GMI Cloud 分も同様に作る
